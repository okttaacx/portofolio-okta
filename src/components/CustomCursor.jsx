...
export default function CustomCursor({
  ...
  useEffect(() => {
    ...
    function hexToRGB(hex) {
      ...
      // Tema gelap (ungu) → multiplier 0.35
      // Tema terang (biru) → multiplier 0.35 agar terlihat soft & adem
      const multiplier = isDarkRef.current ? 0.35 : 0.35;
      return { r: r * multiplier, g: g * multiplier, b: b * multiplier };
    }

    function generateColor() {
      if (isDarkRef.current) {
        // Tema gelap → ungu
        const shades = ['#7c3aed', '#8b5cf6', '#a855f7', '#c084fc', '#9333ea'];
        return hexToRGB(shades[Math.floor(Math.random() * shades.length)]);
      } else {
       const shades = ['#3b82f6', '#60a5fa', '#2563eb', '#38bdf8', '#0ea5e9'];
        return hexToRGB(shades[Math.floor(Math.random() * shades.length)]);
      }
    }
